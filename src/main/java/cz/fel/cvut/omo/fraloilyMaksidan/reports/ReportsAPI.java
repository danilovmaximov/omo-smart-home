...
public class ReportsAPI {
     ...
     * Sets new relative path for writing file
     */
    ...
    public void configurationReportPrint(boolean withBorders) {
        String border = withBorders ?
                "=================== House configuration ===================" : "";
        if(withBorders) System.out.println(border);
    ...
    public void consumptionReportPricePrint(String filename, int gas, int water, int electricity) {
        printJSONtoFile(filename, reports.getConsumptionReport().getAllConsumptions(gas, water, electricity));
    }
    ...
    /**
     * @param withBorder
     * Calls every existing API method to print info to the terminal.
     */
    ...
    /**
     * @param filename
     * Calls every existing API method to print into to the given file.
     */
    ...
    private void printJSONtoFile(String filename, Object map) {
        try {
            (new ObjectMapper()).writerWithDefaultPrettyPrinter().writeValue(Paths.get(ReportsAPI.relative + filename).toFile(), map);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
